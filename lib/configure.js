...
function configure (gyp, argv, callback) {
  ...
  function runGyp (err) {
    ...
    if (process.platform === 'os390') {
      var logprefix = "find zoslib's zos-base.h:"
      var msg
      var zoslibIncTmp = process.env.ZOSLIB_INCLUDES
      if (zoslibIncTmp) {
        zoslibIncTmp = findAccessibleSync(logprefix, zoslibIncTmp, ['zos-base.h'])
        if (zoslibIncTmp === undefined) {
          msg = msgFormat('Could not find zos-base.h file in the directory set ' +
                          'in ZOSLIB_INCLUDES environment variable: %s',
        ...
        zoslibIncTmp = findAccessibleSync(logprefix, nodeRootDir, candidates)
        if (zoslibIncTmp === undefined) {
          msg = msgFormat('Could not find any of %s in directory %s; set ' +
                          'environmant variable ZOSLIB_INCLUDES to the path ' +
                          'that contains zos-base.h', candidates.toString(),
                          nodeRootDir);
        }
      }
      if (zoslibIncTmp !== undefined) {
        zoslibIncDir = path.dirname(zoslibIncTmp)
